...
def main_worker(gpu, args):
    ...
    #
    processed_adata = preprocess_dataset(traindir, labeldir, select_highly_variable_gene=False, do_preprocess=args.CPM)
    ...
    #processed_adata.obs['x'] = processed_adata.obs['assigned_cluster']
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch>=args.warmup_epoch and epoch%args.save_freq==0:
            ...
            if epoch%args.save_freq==0 and epoch<300:
                ...
                sklearn_kmeans_metrics = run_sklearn_kmeans(features, labels, int(args.num_cluster[0]))
                ...
                with open(os.path.join(args.exp_dir, f'result.txt'), "a") as f:
                    f.writelines(f"{epoch}\t" + '\t'.join((str(elem) for elem in sklearn_kmeans_metrics)) + "\n")
...
def preprocess_dataset(traindir: str="", 
    ...
    count_frame = pd.read_csv(traindir)
    meta_frame = pd.read_csv(labeldir)
    ...
    if select_highly_variable_gene:
