...
def main_worker(gpu, args):
    ...
    #
    processed_adata = preprocess_dataset(traindir, labeldir, select_highly_variable_gene=args.highlyGene, do_CPM=args.CPM, do_log=args.log, drop_prob=args.drop_prob)
    ...
    save_path = os.path.join(save_path, "Scarlet")
    if os.path.exists(save_path)!=True:
        os.makedirs(save_path)
    txt_path = os.path.join(save_path, "metric_Scarlet.txt")
    f = open(txt_path, "a")
    f.write("{} {} {}\n".format(dataset_name, best_ari, best_nmi))
    f.close()

    # save feature & label
    np.savetxt(os.path.join(save_path, "feature_Scarlet_{}.csv".format(dataset_name)), best_features, delimiter=',')
    label_decoded = [train_dataset.label_decoder[i] for i in save_labels]
    save_labels_df = pd.DataFrame(label_decoded, columns=['x'])
    save_labels_df.to_csv(os.path.join(save_path, "gt_label_Scarlet_{}.csv".format(dataset_name)))

    pd_labels_df = pd.DataFrame(best_pd_labels, columns=['kmeans'])
    pd_labels_df.to_csv(os.path.join(save_path, "pd_label_Scarlet_{}.csv".format(dataset_name)))
...
def preprocess_dataset(traindir: str="", 
                       labeldir: str="",
