...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        cluster_result = None
        if epoch>=args.warmup_epoch and epoch%1==0:
            ...
            if args.gpu == 0:
                ...
                sklearn_kmeans_ari = run_sklearn_kmeans(features, labels)
                with open(os.path.join(args.exp_dir, f'result.txt'), "a") as f:
                    f.writelines(f"{epoch}\t{ari}\t{sklearn_kmeans_ari}\n")
...
def compute_features(eval_loader, model, args):
    ...
    for i, (images, index, label) in enumerate(tqdm(eval_loader)):
        
        with torch.no_grad():
            images = images.cuda(non_blocking=True)
            feat = model(images,is_eval=True) 
            features[index] = feat
            labels[index] = label.cuda(non_blocking=True)
...
def run_sklearn_kmeans(features, labels):
    ...
    print(f"The sklearn ARI is {ari}\n")
    return ari
