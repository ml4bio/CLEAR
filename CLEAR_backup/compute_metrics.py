...
def traverse_folder_compute_metrics_with_KMEANS(root_path, save_path):
    for filename in os.listdir(root_path):
        ...
        feature_path = os.path.join(root_path, filename.replace("label", "feature"))
        gt_label = pd.read_csv(gt_label_path, index_col=0).iloc[:, 0]
        feature = pd.read_csv(feature_path, index_col=0).iloc[:, 0]

        random_seed = 0
        num_cluster = gt_label.unique().shape[0]
        pd_label = KMeans(n_clusters=num_cluster, random_state=random_seed).fit(feature)

        pd_label_path = gt_label_path.replace("label", "pd_label")
        pd_label.to_csv(os.path.join(save_path, pd_label_path))
        ...
        csv_path = os.path.join(save_path, "new_metrics_Scarlet.csv")
        need_head = False
...
if __name__=="__main__":
    ...
    if len(sys.argv) == 3:
        ...
        traverse_folder_compute_metrics_with_KMEANS(root_path, save_path)
    else:
